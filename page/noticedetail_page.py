...
class DetailNotice(BasePage):
    ...
    def title_notice(self,btcontent):
        '''添加公告的详细页面中的标题元素定位'''
        self.find_element(self.title_notice_locator).send_keys(btcontent)

    def nrcontent_notice(self,nrcontent):
        self.driver.switch_to.frame(self.driver.find_elements_by_tag_name('iframe')[0])
        '''添加公告的详细页面中的内容元素定位'''
        self.find_element(self.nrcontent_notice_locator).send_keys(nrcontent)
    ...
    def alterbiaoti_notice(self,alcontent):
        ...
        self.find_element(self.altercontent_notice_locator).send_keys(alcontent)
